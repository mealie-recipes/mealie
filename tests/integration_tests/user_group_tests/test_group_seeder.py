...
def test_seed_foods(api_client: TestClient, unique_user: TestUser):
    ...
    foods = database.ingredient_foods.page_all(PaginationQuery(page=1, per_page=-1))
    assert len(foods) == 0
...
def test_seed_units(api_client: TestClient, unique_user: TestUser):
    ...
    # Check that the foods was created
    units = database.ingredient_units.page_all(PaginationQuery(page=1, per_page=-1))
    assert len(units) == 0
...
def test_seed_labels(api_client: TestClient, unique_user: TestUser):
    ...
    # Check that the foods was created
    labels = database.group_multi_purpose_labels.page_all(PaginationQuery(page=1, per_page=-1))
    assert len(labels) == 0
