<template>
  ...
  >
    <template v-if="state.ctx.state === States.Initial">
      <v-card width="600px">
        <v-toolbar width="100%" color="primary" class="d-flex justify-center" style="margin-bottom: 5rem" dark>
        ...
        <div class="d-flex pa-4" style="gap: 1em">
    ...
    <template v-else-if="state.ctx.state === States.ProvideToken">
      <v-card width="600px">
        ...
        <v-divider></v-divider>
        <v-card-text>
          Please provide the registration token associated with the group that you'd like to join. You'll need to obtain
          this from an existing group member.
          <v-form ref="domTokenForm" class="mt-4">
        ...
        <v-card-text>
          <v-form ref="domGroupForm">
            ...
            <div class="mt-n4 px-2">
              ...
              <p class="text-caption mt-n4">
                Mealie ships with a collection of Foods, Units, and Labels that can be used to populate your group with
                helpful data for organizing your recipes.
              </p>
            </div>
          </v-form>
        </v-card-text>
      ...
      <v-card width="600px">
        ...
        <v-card-text>
          <v-form ref="domAccountForm">
            ...
            />
            <div class="mt-n4 px-2">
              <v-checkbox v-model="accountDetails.advancedOptions.value" label="Enable Advanced Content"></v-checkbox>
              <p class="text-caption mt-n4">
    ...
    <template v-else-if="state.ctx.state === States.ProvideCredentials">
      <v-card width="600px">
        <v-card-title>
          <v-icon large class="mr-3"> {{ $globals.icons.user }}</v-icon>
        ...
        <v-card-actions class="justify-space-between">
          ...
          <BaseButton icon-right @click="credentials.next">
            <template #icon> {{ $globals.icons.forward }}</template>
            Next
          </BaseButton>
        </v-card-actions>
        ...
        <v-list>
          ...
          <v-list-item>
            <v-list-item-content>
              ...
              <v-list-item-subtitle> {{ accountDetails.advancedOptions.value ? "Yes" : "No" }} </v-list-item-subtitle>
            </v-list-item-content>
          </v-list-item>
        </v-list>
  ...
  setup() {
    ...
    const accountDetails = {
      ...
      next: () => {
        ...
        state.setState(States.ProvideCredentials);
      },
    };
    ...
    const domCredentialsForm = ref<VForm | null>(null);
    ...
    const credentials = {
      ...
      password2,
      next: () => {
        if (!safeValidate(domCredentialsForm as Ref<VForm>)) {
          return;
        }
