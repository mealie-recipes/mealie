...
def get_auth_provider(session: Session, request: Request) -> AuthProvider:
    settings = get_app_settings()

    if request.cookies.get("mealie.auth.strategy") == "oidc":
        return OpenIDProvider(session, request)

    if settings.LDAP_ENABLED:
        return LDAPProvider(session, request)

    return CredentialsProvider(session, request)
