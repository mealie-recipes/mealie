...
class IngredientUnitModel(SqlAlchemyBase, BaseMixins):
    ...
    use_abbreviation: Mapped[bool] = mapped_column(Boolean, default=False)
    fraction: Mapped[bool] = mapped_column(Boolean, default=True)
...
class IngredientFoodModel(SqlAlchemyBase, BaseMixins):
    ...
    label_id: Mapped[GUID] = mapped_column(GUID, ForeignKey("multi_purpose_labels.id"))
    label: Mapped[MultiPurposeLabel] = orm.relationship(MultiPurposeLabel, uselist=False, back_populates="foods")
...
class RecipeIngredient(SqlAlchemyBase, BaseMixins):
    ...
    recipe_id: Mapped[GUID] = mapped_column(GUID, ForeignKey("recipes.id"))

    title: Mapped[str] = mapped_column(String)  # Section Header - Shows if Present
    note: Mapped[str] = mapped_column(String)  # Force Show Text - Overrides Concat

    # Scaling Items
    unit_id: Mapped[GUID] = mapped_column(GUID, ForeignKey("ingredient_units.id"))
    unit: Mapped[IngredientUnitModel] = orm.relationship(IngredientUnitModel, uselist=False)

    food_id: Mapped[GUID] = mapped_column(GUID, ForeignKey("ingredient_foods.id"))
    ...
    reference_id: Mapped[GUID] = mapped_column(GUID)  # Reference Links
