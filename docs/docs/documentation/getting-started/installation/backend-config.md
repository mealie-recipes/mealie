...
| REVERSE_PROXY_AUTH_SIGNUP_ENABLED   |        False        | Enables new users to be created automatically the first time they're seen in the trusted header                                                                                                                                      |
| REVERSE_PROXY_AUTH_TRUSTED_IPS      |         `""`         | Comma-separated hosts or CIDRs allowed to originate the reverse proxy auth header. Strongly recommended in production; requests from any other address are rejected.                                                                |
